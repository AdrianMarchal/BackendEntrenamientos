...
const getAllWorkouts = (filterParams) => {
    try{
        ...
        if(filterParams.modo && filterParams.limit && filterParams.limit < DB.entrenamientos.length){
            let copia = DB.entrenamientos.filter((workout) => workout.modo.toLowerCase().includes(filterParams.modo.toLowerCase()))
            return copia.slice(0,filterParams.limit)
        }
        if(filterParams.modo){
            return DB.entrenamientos.filter((workout) => 
            workout.modo.toLowerCase().includes(filterParams.modo.toLowerCase()))
        }
        if(filterParams.limit && filterParams.limit < DB.entrenamientos.length){
            return DB.entrenamientos.splice(0,filterParams.limit)
        }
        return workouts
    } catch(error){
